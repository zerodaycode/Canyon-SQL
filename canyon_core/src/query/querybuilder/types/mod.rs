...
mod __impl {
    ...
    pub(crate) fn check_invariants_over_condition_clauses<'a, 'b, P: AstProcessor + ToSql<'a>>(_self: &'a QueryBuilder<'a, P>) -> Result<(), Box<dyn Error + Send + Sync + 'b>> {
        let _self = super::__validators::check_where_clause_position(_self)?;
        Ok(())
    }

    pub(crate) fn create_condition_clause<'a, 'b, P: AstProcessor + ToSql<'a>>(_self: &mut QueryBuilder<'a, P>, kind: ConditionClauseKind, column_name: impl Into<ColumnRef<'a>>, operator: Comp) {
...
mod __validators {
    ...
    pub(crate) fn check_where_clause_position<'a, 'b, P: AstProcessor + ToSql<'a>>(_self: &QueryBuilder<'a, P>) -> Result<(), Box<dyn Error + Send + Sync + 'b>> {
        if let Some(condition_clause) = &_self.base_ast.conditions.first() && condition_clause.kind.ne(&ConditionClauseKind::Where) {
