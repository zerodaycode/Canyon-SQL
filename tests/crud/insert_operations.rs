...
async fn test_crud_insert_operation() {
    ...
    let inserted_league = League::find_by_pk(&new_league.id).await
...
async fn test_crud_insert_datasource_operation() {
    ...
    // We insert the instance on the database, on the `League` entity
    new_league.insert_datasource(PSQL_DS).await.expect("Failed insert datasource operation");
...
async fn test_crud_multi_insert_operation() {
    ...
    new_league_mi.insert_datasource(PSQL_DS).await.expect("Failed insert datasource operation");
    new_league_mi_2.insert_datasource(PSQL_DS).await.expect("Failed insert datasource operation");
    new_league_mi_3.insert_datasource(PSQL_DS).await.expect("Failed insert datasource operation");

    // Recover the inserted data by primary key
    let inserted_league = League::find_by_pk(&new_league_mi.id).await
        .expect("[1] - Failed the query to the database")
        .expect("[1] - No entity found for the primary key value passed in");
    let inserted_league_2 = League::find_by_pk(&new_league_mi_2.id).await
        .expect("[2] - Failed the query to the database")
        .expect("[2] - No entity found for the primary key value passed in");
    let inserted_league_3 = League::find_by_pk(&new_league_mi_3.id).await
