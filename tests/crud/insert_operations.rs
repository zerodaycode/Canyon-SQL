...
fn test_crud_insert_datasource_mysql_operation() {
    ...
    // We insert the instance on the database, on the `League` entity
    let result = new_league.insert_datasource(MYSQL_DS).await;
...
fn test_crud_multi_insert_datasource_mysql_operation() {
    ...
    // Insert the instance as database entities
    let first_insert = new_league_mi.insert().await;
    let second_insert = new_league_mi_2.insert().await;
    let three_insert = new_league_mi_3.insert().await;

    assert!(
        first_insert.is_ok(),
        "Failed first insert datasource operation"
    );
    assert!(
        second_insert.is_ok(),
        "Failed second insert datasource operation"
    );
    assert!(
        three_insert.is_ok(),
        "Failed three insert datasource operation"
    )
}
