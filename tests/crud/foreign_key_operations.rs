...
///! impls `ForeignKeyable` (isn't requiered, but it won't unlock the
...
async fn test_crud_search_by_foreign_key() {
    let some_tournament: Tournament = Tournament::find_by_pk(&1).await
    ...
    let parent_entity: Option<League> = some_tournament.search_league().await
        .expect("Result variant of the query is err");
...
async fn test_crud_search_by_foreign_key_datasource() {
    let some_tournament: Tournament = Tournament::find_by_pk_datasource(&1, PSQL_DS).await
        .expect("Result variant of the query is err")
        .expect("No result found for the given parameter");

    // We can get the parent entity for the retrieved child instance
    let parent_entity: Option<League> = some_tournament.search_league_datasource(PSQL_DS).await
        .expect("Result variant of the query is err");
...
async fn test_crud_search_reverse_side_foreign_key() {
    let some_league: League = League::find_by_pk(&1).await
        .expect("Result variant of the query is err")
        .expect("No result found for the given parameter");
    ...
    child_tournaments.iter().for_each(|t|
        assert_eq!(t.league, some_league.id)
    );
}
...
async fn test_crud_search_reverse_side_foreign_key_datasource() {
    let some_league: League = League::find_by_pk_datasource(&1, PSQL_DS).await
        .expect("Result variant of the query is err")
        .expect("No result found for the given parameter");

    // Computes how many tournaments are poiting to the retrieved league
    let child_tournaments: Vec<Tournament> = Tournament::search_league_childrens_datasource(
        &some_league, PSQL_DS
    ).await
    .expect("Result variant of the query is err");
