...
async fn test_crud_delete_method_operation() {
    ...
    assert_eq!(
        new_league.id,
        League::find_by_pk_datasource(&new_league.id, PSQL_DS).await
    ...
    new_league.delete().await.expect("Failed to delete the operation");
    ...
    assert_eq!(
        League::find_by_pk(&new_league.id).await
...
async fn test_crud_delete_datasource_method_operation() {
    ...
    // We insert the instance on the database, on the `League` entity
    new_league.insert_datasource(PSQL_DS).await.expect("Failed insert operation");
    assert_eq!(
        new_league.id,
        League::find_by_pk_datasource(&new_league.id, PSQL_DS).await
            .expect("Request error")
    ...
    // Now that we have an instance mapped to some entity by a primary key, we can now
    // remove that entry from the database with the delete operation
    new_league.delete_datasource(PSQL_DS).await.expect("Failed to delete the operation");
