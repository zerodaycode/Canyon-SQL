...
async fn test_crud_find_with_querybuilder() {
    ...
    let league_idx_0 = filtered_leagues
        .get(0)
        .unwrap();
...
async fn test_crud_update_with_querybuilder() {
    ...
    League::update_query()
        .set(
            ...
            LeagueFieldValue::id(1), Comp::Gt
        ).and(
            LeagueFieldValue::id(8), Comp::Lt
        ).query()
        .await
    ...
    found_updated_values.iter().for_each(|league|
        assert_eq!(league.slug, "Updated with the QueryBuilder")
    );
}
...
async fn test_crud_update_with_querybuilder_datasource() {
    ...
    Player::update_query_datasource(PSQL_DS)
        .set(
            ...
            PlayerFieldValue::id(1), Comp::Gt
        ).and(
            PlayerFieldValue::id(8), Comp::Lt
        ).query()
        .await
        .expect("Failed to update records with the querybuilder");
}
...
async fn test_crud_delete_with_querybuilder() {
    Tournament::delete_query()
        .r#where(
            TournamentFieldValue::id(14), Comp::Gt
        ).and(
            TournamentFieldValue::id(16), Comp::Lt
        ).query()
        .await
        .expect("Error connecting with the database on the delete operation");

    assert_eq!(
        Tournament::find_by_pk(&15).await.unwrap(),
        None
    );
}
...
async fn test_crud_delete_with_querybuilder_datasource() {
    Player::delete_query_datasource(PSQL_DS)
        .r#where(
            PlayerFieldValue::id(120), Comp::Gt
        ).and(
            PlayerFieldValue::id(130), Comp::Lt
        ).query()
        .await
