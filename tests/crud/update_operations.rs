...
async fn test_crud_update_method_operation() {
    ...
    let mut updt_candidate: League = League::find_by_pk(&1).await
    ...
    updt_candidate.update().await.expect("Failed the update operation");

    // Retrieve it again, and check if the value was really updated
    let updt_entity: League = League::find_by_pk(&1).await
    ...
    updt_candidate.update().await.expect(
        "Failed the restablish initial value update operation"
    );
}
...
async fn test_crud_update_datasource_method_operation() {
    // We first retrieve some entity from the database. Note that we must make
    // the retrieved instance mutable of clone it to a new mutable resource
    let mut updt_candidate: League = League::find_by_pk_datasource(&1, PSQL_DS).await
        .expect("[1] - Failed the query to the database")
        .expect("[1] - No entity found for the primary key value passed in");
    ...
    updt_candidate.ext_id = updt_value;
    updt_candidate.update_datasource(PSQL_DS).await.expect("Failed the update operation");

    // Retrieve it again, and check if the value was really updated
    let updt_entity: League = League::find_by_pk_datasource(&1, PSQL_DS).await
        .expect("[2] - Failed the query to the database")
        .expect("[2] - No entity found for the primary key value passed in");
