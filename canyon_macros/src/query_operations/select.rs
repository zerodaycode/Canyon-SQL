...
pub fn generate_find_by_foreign_key_tokens(macro_data: &MacroTokens<'_>, table_schema_data: &String) -> Vec<TokenStream> {
    let mut fk_quotes = Vec::new();

    for fk_annot in macro_data.get_fk_annotations().iter() {
        println!("FK annot: {:?}", &fk_annot);
    }
        ...
        if let EntityFieldAnnotation::ForeignKey(table, column) = fk_annot {
            ...
            fk_quotes.push( 
                quote! {
                    ...
                    async fn #quoted_method_name(&self) -> 
