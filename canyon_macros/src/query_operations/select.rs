...
pub fn generate_find_all_tokens(
    ...
    use __details::find_all_generators::*;
    ...
    let find_all = create_find_all_macro(ty, &stmt);

    let find_all_datasource = create_find_all_ds_macro(ty, &stmt);

    let find_all_unchecked = create_find_all_unchecked_macro(ty, &stmt);

    let find_all_unchecked_datasource = create_find_all_unchecked_ds_macro(ty, &stmt);
...
#[cfg(test)]
mod macro_builder_find_all_tests {
    use super::__details::find_all_generators::*;
    ...
    fn test_macro_builder_find_all_unchecked() {
        let ty: Ident = Ident::new("User", Span::call_site());
        let find_all_builder = create_find_all_unchecked_macro(&ty, SELECT_ALL_STMT);
        let find_all_ds = find_all_builder
            .generate_tokens()
            .to_string();

        assert!(find_all_ds.contains("async fn find_all_unchecked"));
        assert!(find_all_ds.contains(RAW_RET_TY));
    }

    #[test]
    fn test_macro_builder_find_all_unchecked_datasource() {
        let ty: Ident = Ident::new("User", Span::call_site());
        let find_all_builder = create_find_all_unchecked_ds_macro(&ty, SELECT_ALL_STMT);
        let find_all_ds = find_all_builder
            .generate_tokens()
            .to_string();

        assert!(find_all_ds.contains("async fn find_all_unchecked_datasource"));
        assert!(find_all_ds.contains(RAW_RET_TY));
        assert!(find_all_ds.contains(LT_CONSTRAINT));
        assert!(find_all_ds.contains(DS_PARAM));
    }
