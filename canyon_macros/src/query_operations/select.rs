...
fn generate_find_by_pk_tokens(
    ...
    let find_by_pk_ds = create_find_by_pk_datasource(ty, &stmt, &result_handling);
...
mod __details {
    use crate::query_operations::{
        macro_template::MacroOperationBuilder,
        doc_comments
    };
    use quote::quote;
    ...
    pub mod pk_generators {
        ...
        pub fn create_find_by_pk_macro(ty: &Ident, stmt: &str, result_handling: &TokenStream) -> MacroOperationBuilder {
            MacroOperationBuilder::new()
                .fn_name("find_by_pk")
                .with_lifetime()
                .user_type(ty)
        ...
        pub fn create_find_by_pk_datasource(ty: &Ident, stmt: &str, result_handling: &TokenStream) -> MacroOperationBuilder {
            MacroOperationBuilder::new()
                .fn_name("find_by_pk_datasource")
                .with_datasource_param()
                .user_type(ty)
                .return_type(ty)
                .add_doc_comment(doc_comments::FIND_BY_PK)
...
mod macro_builder_read_ops_tests {
    use super::__details::{find_all_generators::*, count_generators::*, pk_generators::*};
    use proc_macro2::Span;
    use syn::Ident;
    use quote::quote;
    ...
    const OPT_RET_TY_LT: &str = "Result < Option < User > , Box < (dyn std :: error :: Error + Send + Sync + 'a) > >";
    ...
    fn test_macro_builder_find_by_pk() {
        ...
        let find_by_pk = find_by_pk_builder
            .generate_tokens()
            .to_string();
    ...
    #[test]
    fn test_macro_builder_find_by_pk_datasource() {
        let ty: Ident = Ident::new("User", Span::call_site());
        let find_by_pk_ds_builder = create_find_by_pk_datasource(&ty, FIND_BY_PK_STMT, &quote!{});
        let find_by_pk_ds = find_by_pk_ds_builder
            .generate_tokens()
            .to_string();
        println!("{:?}", find_by_pk_ds.split("\n").collect::<Vec<_>>());

        assert!(find_by_pk_ds.contains("async fn find_by_pk_datasource"));
