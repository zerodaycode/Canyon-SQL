...
mod __operations {
    ...
    fn generate_parent_lookup_tokens(
        macro_data: &MacroTokens<'_>,
    ) -> syn::Result<Vec<FkOperationTokens>> {
        let mut operations = Vec::new();

        for (field_ident, annotation) in macro_data.get_fk_annotations() {
            if let EntityFieldAnnotation::ForeignKey(table, column) = annotation {
                ...
                operations.extend([fk_operation, fk_operation_with]);
            }
        }
