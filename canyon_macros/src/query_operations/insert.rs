...
pub fn generate_multiple_insert_tokens(macro_data: &MacroTokens) -> TokenStream {
    ...
    quote! {
        ...
        /// .ok();
        /// ```
        #vis async fn multi_insert(values: & mut Vec<& mut #ty>) -> (
            Result<(), Box<dyn std::error::Error + Sync + std::marker::Send>> 
        ) {
            ...
            let mut final_values: Vec<Vec<Box<&dyn QueryParameters<'_>>>> = Vec::new();
            for instance in v2.iter() {
                let intermediate: &[&dyn QueryParameters<'_>] = &[#(#macro_fields),*];
                
                let mut longer_lived: Vec<Box<&dyn QueryParameters<'_>>> = Vec::new();
            ...
            println!("Autogenerated IDs: {:?}", &autogenerated_ids);
        ...
        //         let intermediate = &[#(#macro_fields_cloned),*];
                
        //         let mut longer_lived = Vec::new();
