...
pub fn generate_read_operations_tokens(
    ...
    let find_all_with = create_find_all_with_macro(&fa_stmt);
...
mod __details {
    ...
    pub mod find_all_generators {
        ...
        pub fn create_find_all_macro(ty: &syn::Ident, stmt: &str) -> TokenStream {
            quote! {
                async fn find_all<R>()
                    -> Result<Vec<R>, Box<(dyn std::error::Error + Sync + Send)>>
                where R: RowMapper, Vec<R>: FromIterator<<R as RowMapper>::Output> {
                    <#ty as canyon_sql::core::Transaction>::query::<&str, R>(
                        #stmt,
                        &[],
                        ""
                    ).await
        ...
        pub fn create_find_all_with_macro(stmt: &str) -> TokenStream {
            quote! {
                async fn find_all_with<'a, R, I>(input: I)
                    -> Result<Vec<R>, Box<(dyn std::error::Error + Sync + Send)>>
                where
                    R: RowMapper,
                    I: canyon_sql::core::DbConnection + Send + 'a,
