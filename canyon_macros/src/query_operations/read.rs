...
pub fn generate_read_operations_tokens(
    ...
    let find_all_tokens =
        generate_find_all_operations_tokens(ty, Some(&ty_generics), mapper_ty, table_schema_data);
    let count_tokens = generate_count_operations_tokens(ty, Some(&ty_generics), table_schema_data);
    let find_by_pk_tokens =
        generate_find_by_pk_operations_tokens(macro_data, Some(&ty_generics), table_schema_data);
...
fn generate_find_all_operations_tokens(
    ...
    let find_all =
        __details::find_all_generators::create_find_all_macro(ty, ty_generics, mapper_ty, &fa_stmt);
    let find_all_with =
...
fn generate_count_operations_tokens(
    ty: &Ident,
    ty_generics: Option<&TypeGenerics>,
    table_schema_data: &String,
) -> TokenStream {
    let count_stmt = format!("SELECT COUNT(*) FROM {table_schema_data}");
    let count = __details::count_generators::create_count_macro(ty, ty_generics, &count_stmt);
        ...
        pub fn create_count_macro(
            ...
            quote! {
                async fn count() -> Result<i64, Box<(dyn std::error::Error + Send + Sync)>> {
                    Ok(<#ty #ty_generics as canyon_sql::core::Transaction>::query_one_for::<&str, &[&dyn canyon_sql::query::QueryParameter<'_>], i64>(#stmt, &[], "").await? as i64)
                }
            }
        }
...
mod macro_builder_read_ops_tests {
    ...
    fn test_create_find_all_macro() {
        ...
        let tokens = create_find_all_macro(&ty, None, &mapper_ty, SELECT_ALL_STMT);
    ...
    fn test_create_count_macro() {
        let ty = syn::parse_str::<Ident>("User").unwrap();
        let tokens = create_count_macro(&ty, None, COUNT_STMT);
        let generated = tokens.to_string();
    ...
    fn test_create_find_by_pk_macro() {
        ...
        let tokens =
            create_find_by_pk_macro(&ty, None, &mapper_ty, FIND_BY_PK_STMT, &pk_runtime_error);
        let generated = tokens.to_string();
