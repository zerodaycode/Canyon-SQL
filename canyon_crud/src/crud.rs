    ...
    where T: Debug + CrudOperations<T> + RowMapper<T>
{
    ...
    async fn __find_by_pk<'a>(table_name: &'a str, pk: &'a str, pk_value: &'a dyn QueryParameters<'a>, datasource_name: &'a str) 
        ...
        println!("Generated by PK: {:?}", &stmt);
        println!("Generated by PK: {:?}", &pk_value);
    ...
    async fn __insert_multi<'a>(
        ...
        primary_key: &'a str,
        fields: &'a str, 
        values_arr: &mut Vec<Vec<Box<&'a dyn QueryParameters<'a>>>>,
        datasource_name: &'a str
    ) -> Result<DatabaseResult<T>, Box<(dyn std::error::Error + Send + Sync + 'static)>> {
        let mut values_arr = values_arr;
        ...
        println!("Multi insert stmt: {:?}", &stmt);
        println!("Multi insert fields: {:?}", fields);
        println!("Multi insert fields values: {:?}", &fields_values);
        ...
        //     .into_iter()
        //     .flatten()
        //     .collect::<Vec<&'a dyn QueryParameters<'a>>>();
